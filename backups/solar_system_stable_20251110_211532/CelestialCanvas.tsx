    ...
    const cameraControls = {
      ...
      focusObject: (objectName: string) => {
        ...
        if (targetObject) {
          ...
          let distance = 20; // Default distance
          let cameraOffset = new THREE.Vector3(1, 0.5, 1).normalize(); // Default viewing angle
          const objName = objectName.toLowerCase();
          
          // Special presets for common views
          if (objName === 'sun') {
            // Inner System view - closer to center, see Mercury through Mars
            distance = 25;
            cameraOffset = new THREE.Vector3(1, 0.7, 1).normalize();
          } else if (objName === 'jupiter') {
            // Outer System view - see Jupiter, Saturn, Uranus, Neptune
            distance = 350;
            cameraOffset = new THREE.Vector3(0.8, 0.4, 0.8).normalize();
          } else if (objName === 'neptune') {
            // Wide Shot - see entire solar system including outer planets
            distance = 500;
            cameraOffset = new THREE.Vector3(0.7, 0.5, 0.7).normalize();
          } else if (objName === 'saturn') {
            distance = 120;
            cameraOffset = new THREE.Vector3(1, 0.4, 1).normalize();
          } else if (objName === 'earth' || objName === 'venus' || objName === 'mars') {
            distance = 15;
            ...
            distance = 250;
            cameraOffset = new THREE.Vector3(0.9, 0.4, 0.9).normalize();
          }
      ...
      if (objectData.type === 'planet' && textureLoader) {
        ...
        if (textureUrl) {
          ...
          if (objectData.name === 'Uranus') {
            ...
            // Neptune: Deep azure blue with strong glow
            objectMaterial = new THREE.MeshStandardMaterial({
              color: 0x4169E1,
              emissive: 0x2849B1,
        ...
        // Special handling for each planet with realistic materials
        if (objectData.name === 'Mercury') {
          objectMaterial = new THREE.MeshStandardMaterial({
            color: 0x8C7853, // Rocky gray-brown
            emissive: 0x4A3F2F,
            emissiveIntensity: 0.15,
            metalness: 0.05,
            roughness: 0.95 // Very rough, cratered surface
          });
        } else if (objectData.name === 'Venus') {
          objectMaterial = new THREE.MeshStandardMaterial({
            color: 0xFFC649, // Bright yellowish
            emissive: 0xFFAA33,
            emissiveIntensity: 0.5, // Very bright due to thick atmosphere
            metalness: 0.1,
            roughness: 0.3 // Smooth cloud tops
          });
        } else if (objectData.name === 'Earth') {
          objectMaterial = new THREE.MeshStandardMaterial({
            color: 0x2B65EC, // Ocean blue with continents
            emissive: 0x1A4DB8,
            emissiveIntensity: 0.25,
            metalness: 0.2, // Slight metallic from water reflection
            roughness: 0.6
          });
        } else if (objectData.name === 'Mars') {
          objectMaterial = new THREE.MeshStandardMaterial({
            color: 0xCD5C5C, // Rusty red
            emissive: 0x8B3A3A,
            emissiveIntensity: 0.2,
            metalness: 0.05,
            roughness: 0.9 // Very rough, dusty surface
          });
        } else if (objectData.name === 'Jupiter') {
          objectMaterial = new THREE.MeshStandardMaterial({
            color: 0xC88B3A, // Tan with red-brown bands
            emissive: 0xA87028,
            emissiveIntensity: 0.35,
            metalness: 0.15,
            roughness: 0.5 // Banded cloud structure
          });
        } else if (objectData.name === 'Saturn') {
          objectMaterial = new THREE.MeshStandardMaterial({
            color: 0xFAD5A5, // Pale gold
            emissive: 0xDAB585,
            emissiveIntensity: 0.3,
            metalness: 0.1,
            roughness: 0.6
          ...
          objectMaterial = new THREE.MeshStandardMaterial({
            color: 0x4FD0E7, // Cyan-blue
            emissive: 0x2FB0C7,
            emissiveIntensity: 0.5,
            metalness: 0.3,
            roughness: 0.4 // Smooth, featureless atmosphere
          });
        } else if (objectData.name === 'Neptune') {
          objectMaterial = new THREE.MeshStandardMaterial({
            color: 0x4169E1, // Deep azure
            emissive: 0x2849B1,
            emissiveIntensity: 0.6,
            metalness: 0.2,
            roughness: 0.4
          ...
          objectMaterial = new THREE.MeshStandardMaterial({
            color: objectColor,
            emissive: objectColor,
            emissiveIntensity: objectData.type === 'planet' ? 0.4 : 0.25,
