...
const WatchmansView = () => {
  ...
  const loadAllData = async () => {
    try {
      ...
      // Generate AI predictions for celestial events
      const predictionMap = new Map<string, EventPrediction>()
      mockCelestialEvents.forEach(event => {
                      ...
                      <td className="px-6 py-4">
                        {prediction ? (
                          <div className="flex items-center gap-2">
                            <span className={`px-2 py-1 text-xs rounded-full ${getSignificanceColor(prediction.predictedSignificance)}`}>
                              {prediction.predictedSignificance}
                            </span>
