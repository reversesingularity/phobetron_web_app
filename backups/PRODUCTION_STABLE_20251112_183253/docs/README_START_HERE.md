...
### UI Design System (OFFICIAL COLOR SCHEME)
**This color scheme is MANDATORY for all phases of development.**

#### Text Colors (for dark space-themed backgrounds)
- **Headings/Titles**: `text-white` or `text-gray-100` (highest contrast)
- **Labels**: `text-gray-200` (form labels, section headers)
- **Body Text**: `text-gray-300` (descriptions, paragraphs)
- **Code/References**: `text-cyan-300` (scripture refs, technical values)
- **Numbers/Metrics**: `text-blue-400` (statistics, dates)
- **Muted Text**: `text-gray-400` (only for truly secondary info)

#### Background Colors
- **Cards**: `bg-zinc-900` with `border-zinc-800`
- **Input Fields**: `bg-zinc-800` with `border-zinc-700`
- **Hover States**: `hover:bg-zinc-800/50`
- **Page Background**: `bg-black` (space theme)

#### Badge Colors (Consistent Across All Features)
- **Status Badges**:
  - Fulfilled: `bg-green-500/10 text-green-400 ring-green-500/20`
  - Partially Fulfilled: `bg-yellow-500/10 text-yellow-400 ring-yellow-500/20`
  - Unfulfilled: `bg-red-500/10 text-red-400 ring-red-500/20`
- **Significance Badges**:
  - Critical: `bg-red-500/10 text-red-400 ring-red-500/20`
  - High: `bg-purple-500/10 text-purple-400 ring-purple-500/20`
  - Medium: `bg-blue-500/10 text-blue-400 ring-blue-500/20`
  - Low: `bg-zinc-500/10 text-zinc-400 ring-zinc-500/20`
- **Category Badges**:
  - Celestial Sign: `bg-cyan-500/10 text-cyan-400 ring-cyan-500/20`
  - Terrestrial Event: `bg-yellow-500/10 text-yellow-400 ring-yellow-500/20`
  - Spiritual Event: `bg-purple-500/10 text-purple-400 ring-purple-500/20`

#### Form Components (Standard Classes)
```tsx
// Labels
...
// Buttons (Catalyst defaults are good)
<Button color="cyan">Primary Action</Button>
<Button color="zinc" outline>Secondary Action</Button>
```
...
// Body Cells
<TableCell className="text-white">Important Value</TableCell>
<TableCell className="text-gray-300">Regular Text</TableCell>
<TableCell className="text-cyan-300">Code/Reference</TableCell>
```
...
<Card>
  ...
  <CardContent>
    <p className="text-gray-300">Body text content</p>
  </CardContent>
</Card>
```

#### Rationale
- **Catalyst UI** uses Tailwind's dark mode variants, but these may not provide sufficient contrast against space-themed black backgrounds (rgb(0,0,0))
- **Explicit text colors** are required to ensure readability
- **Zinc palette** (800/900) provides better contrast than pure black for inputs/cards
- **Consistent badge colors** ensure users learn the color-coding system across all features
- **This scheme was validated** on the Prophecy Codex enhanced page after user reported poor text visibility
