 ...
 * 
 * Main navigation menu with links to all major modules.
 * Features active route highlighting and alert badge.
 */
...
export default function Sidebar() {
  ...
  // const { activeAlerts } = useAlerts({ status: 'ACTIVE' });
  const activeAlerts = 0; // Temporary: disable alerts fetching
    ...
    <aside className="fixed inset-y-0 left-0 z-50 w-64 border-r border-zinc-800 bg-zinc-900">
      ...
      <div className="flex h-16 items-center border-b border-zinc-800 px-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-bold text-zinc-50">Celestial Signs</div>
            <div className="text-xs text-zinc-400">Observatory</div>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 space-y-1 px-3 py-4">
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <span className={isActive ? 'text-blue-500' : 'text-zinc-500 group-hover:text-zinc-400'}>
                  {item.icon}
                </span>
                <span>{item.name}</span>
              </div>
      ...
      <div className="border-t border-zinc-800 p-4">
        <div className="rounded-lg bg-zinc-800/50 p-3">
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="text-zinc-400">API Connected</span>
          </div>
          <div className="mt-2 text-xs text-zinc-500" suppressHydrationWarning>
