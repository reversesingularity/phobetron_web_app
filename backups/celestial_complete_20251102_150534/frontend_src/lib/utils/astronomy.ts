 ...
 * Colors based on astronomical observations and NASA imagery
 */
export const PLANET_CONFIGS: Record<string, PlanetConfig> = {
  mercury: { name: 'Mercury', size: 0.38, color: '#F0C050' }, // Golden-brown color from surface composition
  venus: { name: 'Venus', size: 0.95, color: '#F5E3C3' },     // Pale yellow from thick atmosphere
  earth: { name: 'Earth', size: 1.0, color: '#339AFF' },      // Blue oceans with clouds
  mars: { name: 'Mars', size: 0.53, color: '#FF0000' },       // Red from iron oxide surface
  jupiter: { name: 'Jupiter', size: 11.2, color: '#FF9900' }, // Orange-brown from atmospheric bands
  saturn: { name: 'Saturn', size: 9.45, color: '#FFCC00', hasRings: true, ringColor: '#DAA520' }, // Yellow-gold atmosphere with icy rings
  uranus: { name: 'Uranus', size: 4.0, color: '#2EC0AA' },    // Cyan-green from methane atmosphere
  neptune: { name: 'Neptune', size: 3.88, color: '#416FE1' }, // Deep blue from methane absorption
};
