...
export default function WatchmanDashboardPage() {
  ...
  const [isStatsOpen, setIsStatsOpen] = useState(false); // Earth Monitor stats - minimized by default
  const [isControlsOpen, setIsControlsOpen] = useState(false); // Layer controls - minimized by default
      ...
      <div className="relative h-[calc(100vh-4rem)] bg-zinc-950 overflow-hidden">
        ...
        <div className="absolute left-6 top-6 z-20">
          ...
          <button
            ...
            className="mb-2 flex h-10 w-10 items-center justify-center rounded-full bg-cyan-600 border border-cyan-500 text-white shadow-lg hover:bg-cyan-700 transition-all ring-2 ring-white"
            aria-label={isStatsOpen ? "Close Earth Monitor stats" : "Open Earth Monitor stats"}
          >
            {isStatsOpen ? (
          ...
          <motion.div 
            className={`
              w-80 space-y-4 transition-all duration-300 ease-in-out
              ${isStatsOpen 
            ...
            animate={{ opacity: isStatsOpen ? 1 : 0, x: isStatsOpen ? 0 : -20 }}
        ...
        <div className="absolute bottom-6 right-6 z-20">
          ...
          <button
            ...
            aria-label={isControlsOpen ? "Close layer controls" : "Open layer controls"}
          >
            {isControlsOpen ? (
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
          ...
          <motion.div 
            className={`
              w-80 rounded-xl border border-cyan-500/20 bg-linear-to-br from-zinc-900/90 to-zinc-950/90 backdrop-blur-xl shadow-lg transition-all duration-300 ease-in-out
              ${isControlsOpen 
                ? 'max-h-[500px] p-5 opacity-100 scale-100' 
                : 'max-h-0 p-0 opacity-0 scale-95 pointer-events-none'
              }
            `}
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: isControlsOpen ? 1 : 0, x: isControlsOpen ? 0 : 20 }}
