...
export default function TimeControlsPanel({
  ...
  // Sync customSpeed state with speedMultiplier prop changes
  useEffect(() => {
  ...
  const handleCustomSpeedSubmit = () => {
    ...
    if (!isNaN(speed) && speed > 0) {
      ...
      setCustomSpeed(speedMultiplier.toString()); // Reset to current speed
    }
  };

  // Handle canceling custom speed input
  const handleCustomSpeedCancel = () => {
    setCustomSpeed(speedMultiplier.toString()); // Reset to current speed
    setShowSpeedInput(false);
  };
        ...
        {showSpeedInput ? (
          <div className="space-y-2">
            <div className="flex gap-2">
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === 'Escape') handleCustomSpeedCancel();
                }}
              ...
              <button
                onClick={handleCustomSpeedCancel}
            ...
            <div className="text-xs text-zinc-500">
              Current: <span className="text-cyan-400 font-medium">{speedMultiplier}x</span> • Enter any speed greater than 0 (e.g., 10000x for fast-forward)
            </div>
          </div>
        ) : (
          <>
